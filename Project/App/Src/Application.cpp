...
#include "Security.h"   /* Week 7 — security module */
...
#include "cmsis_os.h"     /* osKernelGetTickCount() for rate limiter */
}
...
void Loop()
{
	/* ---- Week 7 Security: periodic stack health report ---- */
	static uint32_t lastStackCheckMs = 0U;
	const uint32_t nowMs = osKernelGetTickCount();
	if ((nowMs - lastStackCheckMs) >= SEC_STACK_CHECK_PERIOD_MS)
	{
		lastStackCheckMs = nowMs;
		SEC_StackWatermarkCheck();
	}
	...
	if (parser.commandReady != 0U)
	{
		...
		/* ---- Week 7 Security: Rate limiting (DoS / Availability) ---- */
		if (!SEC_CommandRateLimiter())
		{
			return;  /* command dropped — rate limit exceeded */
		}

		if (command.Header == Header_PWMControl)
		{
			/* ---- Week 7 Security: Input validation (Integrity) ---- */
			(void) SEC_ValidatePwmValue(command.Cp.PWMControl.Pwm);
			(void) PWM_SetLedRedDuty(command.Cp.PWMControl.Pwm);
		}
		else if (command.Header == Header_MotorControl)
		{
			/* ---- Week 7 Security: Clamp motor angles to safe range ---- */
			(void) SEC_ValidateMotorAngle(&command.Cp.MotorControl.Motor1AngleDeg);
			(void) SEC_ValidateMotorAngle(&command.Cp.MotorControl.Motor2AngleDeg);

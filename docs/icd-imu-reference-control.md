...
- `imu_reference_orientation_targets`: sends three `imuReferenceControl` targets, then waits until `imuReferenceStatus.azimuthErrorDeg/elevationErrorDeg` stay within `toleranceDeg` for `stableMs`. It disables IMU reference control at the end by default.
